...
| `next_stop`      | The ID of next stop or station. Updated on each departure from or passing of a stop. `EOL` (end of line) after final stop and `NA` if the vehicle is leaving HSL area. Matches `stop_id` in GTFS (value of `gtfsId` field, without `HSL:` prefix, in **Stop** type in the routing API).

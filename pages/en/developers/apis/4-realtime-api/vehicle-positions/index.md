...
| `direction_id`   | The line direction of the trip, either `1` or `2`.<br/>**Note:** This does not exactly match `direction_id` in GTFS or [the Routing API](../../1-routing-api/).<br/>Value `1` here is same as `0` in GTFS and the Routing API.<br/>Value `2` here is same as `1` in GTFS and the Routing API.
